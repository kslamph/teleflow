...
func main() {
	...
	registrationFlow, err := teleflow.NewFlow("user_registration").
		...
		Process(func(ctx *teleflow.Context, input string, buttonClick *teleflow.ButtonClick) teleflow.ProcessResult {
			log.Println("Welcome step processed with input:", input)
			...
			log.Printf("User name set to: %s", input)
		...
		OnComplete(func(ctx *teleflow.Context) error {
			log.Println("Registration flow completed successfully")
			name, _ := ctx.GetFlowData("user_name")
			age, _ := ctx.GetFlowData("user_age")
			log.Printf("Registration flow completed successfully for user: %s, age: %s", name, age)
